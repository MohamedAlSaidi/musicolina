...
@Client.on_message(command(["انضم","userbotjoin","join", f"انضم@Olinyomu", f"userbotjoin@Olinyomu", f"join@Olinyomu"]) & ~filters.private & ~filters.bot)
...
async def addchannel(client, message):
    ...
    try:
        ...
        print(e)
        await message.reply_text(
                        f"حدث خطأ ما\n{e}\n\nيرجي اعادة توجية هذة الرسالة الي المطور @Aashekelsafar \n\nقم بي اضافه الحساب المساعد يدويا @Olinyomu")
        return
...
@USER.on_message(command(["ubleave","userbotleave","leave", f"ubleave@Olinyomu", f"userbotleave@Olinyomu", f"leave@Olinyomu"]) & ~filters.private & ~filters.bot)
@authorized_users_only
async def rem(USER, message):
    try:
        ...
        await message.reply_text("حدث خطأ ما\n{e}\n\nيرجي اعادة توجية هذة الرسالة الي المطور @Aashekelsafar \n\nقم بي طرد الحساب المساعد يدويا @Olinyomu")
        return


@Client.on_message(command(["غادر","userbotleaveall","leaveall", f"غادر@Olinyomu", f"userbotleaveall@Olinyomu", f"leaveall@Olinyomu"]) & ~filters.private & ~filters.bot)
